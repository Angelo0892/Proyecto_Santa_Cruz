...
class SistemaExpertoOncologico:
    ...
    def cargar_modelos(self):
        try:
            self.modelo_diagnostico = joblib.load('modelo_diagnostico.joblib')
            self.modelo_estadio = joblib.load('modelo_estadio.joblib')
        except FileNotFoundError:
            self.modelo_diagnostico = RandomForestClassifier(n_estimators=100, random_state=42)
            self.modelo_estadio = MLPClassifier(hidden_layer_sizes=(100, 50), max_iter=1000, random_state=42)

    def diagnosticar(self, paciente):
        caracteristicas = self.preparar_caracteristicas(paciente)
        tipo_cancer = self.modelo_diagnostico.predict([caracteristicas])[0]
        estadio = self.modelo_estadio.predict([caracteristicas])[0]
        return Cancer(tipo_cancer, estadio)

    def preparar_caracteristicas(self, paciente):
        return [
            paciente.edad,
            1 if paciente.genero == "masculino" else 0,
            1 if "tos cronica" in paciente.sintomas else 0,
            1 if "perdida de peso" in paciente.sintomas else 0,
            1 if "dolor abdominal" in paciente.sintomas else 0,
            1 if "fatiga" in paciente.sintomas else 0,
            paciente.resultados_pruebas.get("tamaño_tumor", 0),
            paciente.resultados_pruebas.get("nivel_marcador_tumoral", 0),
            paciente.resultados_pruebas.get("nivel_hemoglobina", 0),
            1 if paciente.antecedentes_familiares else 0,
            1 if "tabaquismo" in paciente.habitos else 0,
            1 if "alcohol" in paciente.habitos else 0
        ]
